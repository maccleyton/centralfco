...
(() => {
  ...
  async function consultRecipient(event) {
    ...
    try {
      ...
      if (missing.length) throw new Error(`A consulta não retornou ${missing.join(', ')}. Confira o cadastro do CNPJ antes de gerar.`);
      ...
      setMessage(error.message || 'Não foi possível consultar os dados deste CNPJ.', 'error');
    } finally {
  ...
  function postalHtml(sender, target, logo, neighborAuthorized) {
    ...
    </style></head><body><div class="toolbar"><strong>AR e Etiqueta dos Correios · A4 horizontal</strong><button onclick="window.print()">Imprimir / salvar PDF</button></div>
  ...
  function generatePostalDocuments() {
    ...
    try {
      ...
      const documentUrl = URL.createObjectURL(new Blob([postalHtml(sender, recipient, logo, neighborAuthorized)], { type: 'text/html;charset=utf-8' }));
      const popup = window.open(documentUrl, '_blank');
      if (!popup) {
        URL.revokeObjectURL(documentUrl);
        throw new Error('O navegador bloqueou a abertura do documento. Permita pop-ups e tente novamente.');
      }
