...
const reportFrameShell = document.getElementById('reportFrameShell');
let loaded = false;
...
function expireReport() {
  ...
  reportFrameShell.style.height = '0';
...
reportFrame.addEventListener('load', async () => {
  ...
  const syncFrameLayout = () => {
    ...
    reportFrame.style.height = `${naturalHeight}px`;
    reportFrame.style.transform = `scale(${scale})`;
    reportFrameShell.style.height = `${Math.ceil(naturalHeight * scale)}px`;
  };
  reportFrame.style.display = 'block';
  syncFrameLayout();
  requestAnimationFrame(() => requestAnimationFrame(syncFrameLayout));
  frameResizeObserver?.disconnect();
  frameResizeObserver = new ResizeObserver(syncFrameLayout);
  ...
  shellResizeObserver?.disconnect();
  shellResizeObserver = new ResizeObserver(syncFrameLayout);
  shellResizeObserver.observe(reportFrameShell);
